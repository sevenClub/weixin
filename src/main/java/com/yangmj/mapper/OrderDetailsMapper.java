...
//    List<Object> selectConversationList(@Param("userId") String userId, @Param("offset") int offset, @Param("limit") int limit);
     ...
     * @return
     */
    @Select("SELECT item.id,item.sport_title sportTitle,item.fee_tags feeTags,item.acture_start_tm actureStartTm,item.game_location gameLocation,cu.avatar_url avatarUrl,item.description,cu.phone,project.sport_img_url sportImgUrl,cu.nickname nickName,item.total_Num totalNum,cu.wechat_openid openId,item.project_cost projectCost,detail.is_captain isCaptain,item.is_full isFull from order_item item ,order_details detail,consumer cu,project_item project where item.id = detail.order_id and detail.wechat_openid = cu.wechat_openid and detail.is_active ='0' and project.project_id = item.project_id and item.id =#{id}")
