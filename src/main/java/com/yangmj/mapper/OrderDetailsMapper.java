...
//    List<Object> selectConversationList(@Param("userId") String userId, @Param("offset") int offset, @Param("limit") int limit);
    ...
    @Select("SELECT detail.order_id,detail.wechat_openid from order_item item ,order_details detail where item.id = detail.order_id and item.order_status in ('1','0') and UNIX_TIMESTAMP(item.acture_start_tm)> UNIX_TIMESTAMP(NOW()) order by item.id desc")
    List<OrderDetails> queryJoinedOpenid();
